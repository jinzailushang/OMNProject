...
class Db {
    ...
    private static function connect($host = 'slave') {
        if (C('db.master') == C('db.slave')) {
            if (is_resource(self::$link['slave'])) {
                self::$link['master'] = & self::$link['slave'];
                return;
            } elseif (is_resource(self::$link['master'])) {
        ...
        // fix bug： 之前用object无用，这是个resource
        if (is_resource(self::$link[$host])) {
    ...
    public static function update($table_name, $update_array = array(), $where = '', $host = 'master') {
        ...
        foreach ($update_array as $k => $v) {
            if (is_array($v)) {
                switch ($v['sign']) {
                    case 'increase': $string_value .= " $k = $k + " . $v['value'] . ",";
                        break;
                    case 'decrease': $string_value .= " $k = $k - " . $v['value'] . ",";
                        break;
                    case 'calc': $string_value .= " $k = " . $v['value'] . ",";
                        break;
