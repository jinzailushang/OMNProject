...
/* 私钥加密方法（签名）
 * parameter：$sourcestr 源字符串
 * parameter：$fileName 私钥文件
 * return: 返回加密后的字符串
 */

function sign($sourcestr, $fileName) {
    $key_content = file_get_contents($fileName);

    $key_content = openssl_pkey_get_private($key_content);
    ...
    $crypttext = "";
    openssl_sign($sourcestr, $crypttext, $pubkeyid);
    openssl_free_key($pubkeyid);
    return base64_encode("" . $crypttext);
}

/* 公钥验证方法（验证）
 * parameter：$dataString 源字符串
 * parameter：$signString 需验证的字符串即私钥加密后的字符串
 * parameter：$fileName 公钥文件
 * return: 验证成功返回1，否则返回0
 */

function verify($dataString, $signString, $fileName) {
    $key_content = file_get_contents($fileName);

    $key_content = openssl_pkey_get_public($key_content);
...
/**
 * @version 删除指定目录下所有文件，及其子目录下所有文件
 * @copyright (c) 2014
...
function deleteFileInDir($dir) {
    ...
    if (is_array($list)) {
        foreach ($list as $info) {
            $url = $dir . '/' . $info;
            if (is_dir($url) && $info != '.' && $info != '..') {
                $res = deleteFileInDir($url);
            } else if (is_file($url)) {
                $res = @unlink($url);
            }
        }
...
function postSoapData($data, $soapUrl = '', $method = '') {
    ini_set("soap.wsdl_cache_enabled", "0");
    if (empty($method) || empty($soapUrl)) {
        return false;
    }
    $soap = new SoapClient($soapUrl);
    $data = $soap->$method($data);
 ...
 * @return 要请求的参数数组
 */
function buildRequestPara($para_temp, $TOKEN) {
    //除去待签名参数数组中的空值和签名参数
...
function setNcCookie($name, $value, $expire = '3600', $path = '', $domain = '', $secure = false) {
    ...
    //print_r(array($name, $value, time() + $expire, $path, $domain, $secure));die;
 ...
 */
function showMessage($msg, $url = '', $time = 2000, $msg_type = 'succ', $show_type = 'html',  $is_show = 1) {
    ...
    switch ($show_type) {
        ...
        case 'exception':
            ...
            echo '<dd></dd>';
            echo '<dd>Copyright 2007-2014 XOMS, All Rights Reserved ' . $lang['company_name'] . '</dd>';
            echo '</dl>';
            echo '</body>';
            echo '</html>';

            $files = BASE_DATA_PATH . DS . 'log/add-order-log';

            $files .= '-' . date('Y-m-d', time()) . '.log';

            //file_put_contents($files, date('Y-m-d H:i:s') . '[' . json_encode($msg) . ']' . PHP_EOL . PHP_EOL, FILE_APPEND);
...
function redirect_url($message = '操作成功', $url = '', $tips_type = 'succ', $time = 3) {
    $url = $url ? $url : $_SERVER['HTTP_REFERER'];
	$tips_type = in_array($tips_type, array('succ', 'error')) ? $tips_type : 'error';
    Tpl::setDir('tpl');
    Tpl::output('url', $url);
    Tpl::output('position', $message);
    Tpl::output('time', $time * 1000);
	Tpl::output('tips_type', $tips_type);
    Tpl::showpage('tips', 'index_layout');
    exit;
}
 ...
 */
function ncPriceFormat($price,$a=2) {
    $price = floatval($price);
    $price_format = number_format($price, $a, '.', '');
    return $price_format;
}
 ...
 *
 * @param int	$str
 * @return string	$str_format
 */
function ncWeightFormat($str) {
    $str = floatval($str);
 ...
 * @return string	$price_format
 */
function ncPriceFormatForList($price, $a = true) {
    if ($price >= 10000) {
 ...
 * 获取供应商申请图片
 */
function getSupplierImageUrl($image_name = '') {
    return UPLOAD_SITE_URL . DS . ATTACH_SUPPLIER . DS . $image_name;
}
...
function write_file($filepath, $data, $mode = null) {
    ...
    if ($data === '')
        $data = '\'\'';
    $data = "<?php defined('InOmniWL') or exit('Access Invalid!'); return " . $data . "\n?>";
 ...
 * @return boolean
 */
function read_file_one_value($filepath,$field,$value,$find_field){
    if(file_exists($filepath)){
        $result = include($filepath);
        if(is_array($result)){
...
function pagecmd($cmd = '', $arg = '', $func = '') {
    ...
    switch (strtolower($cmd)) {
        case 'seteachnum': $page->setEachNum($arg);
            break;
        case 'settotalnum': $page->setTotalNum($arg);
            break;
        case 'setstyle': $page->setStyle($arg);
            break;
        case 'show': return $page->show($arg, $func);
            break;
        case 'obj': return $page;
            break;
        case 'gettotalnum': return $page->getTotalNum();
            break;
        case 'gettotalpage': return $page->getTotalPage();
            break;
        default: break;
    }
}
...
function getChat($layout) {
    if (!file_exists(BASE_CORE_PATH . '/framework/libraries/chat.php'))
        return '';
    if (!class_exists('Chat'))
        import('libraries.chat');
...
//    $site_url = empty($site_url) ? SHOP_SITE_URL : $site_url;
    ...
    //将商品、商家、分类、品牌、文章自动生成的伪静态URL使用短URL代替
 ...
 * @param string $op op方法名
 * @param array $args URL其它参数
 * @param string $store_domian 商家二级域名
 * @return string
 */
function urlShop($act = '', $op = '', $args = array(), $store_domain = '') {
    ...
    return url($act, $op, $args, $rewrite_flag, SITE_SITE_URL);
}
...
function checkPlatformStore() {
    return true; //($_SESSION['store_id'] == DEFAULT_PLATFORM_STORE_ID) ? true : false;
}
 ...
 * @param string $model 接口模块
 * @param string $action 接口方法
 * @param string $directory 接口目录
 * @return object
 */
function getApiObject($model, $action, $directory) {
    $filedir = BASE_DATA_PATH . DS . 'api' . DS . $model . DS . 'index.php';
    if ((file_exists($filedir) === FALSE) || (is_readable($filedir) === FALSE) || empty($directory)) {
        return FALSE;
    } else {
        include_once($filedir);
    }
    eval('$api = new ' . $model . '_base();');

    $object = $api->load($directory);
...
function toTimeZone($src, $from_tz = 'Asia/Shanghai', $to_tz = 'Europe/Berlin', $fm = 'Y-m-d H:i:s') {
...
/*
 * 计算德国时间是否是周末或节假日， ($tag, $monat, $jahr 为需要判断的日期) ， 如果返回true， 则日期加一天即可。
 * $tag (string 1-31) 
 * $monat (string 1-12) 
 * $jahr (string YYYY) 
 */
...
/**
 * @version 返回日期 (根据一个时间返回时分秒随机)
 * @copyright (c) 2015-08-11, jack
 */
function get_rand_time($time) {
    $s = rand(7, 24);
    $f = rand(1, 59);
    $m = rand(1, 59);
    return strtotime(date('Y-m-d H', $time) . ':' . sprintf("%02d", $f) . ':' . sprintf("%02d", $m));
}
...
/**
 * 根据条件查找某字段值
 * @param type $params(表，键名，键值，所查字段)
 * SQL语句为：select 所查字段 from 表 where 键名=键值;
 * @return string
 */
function getFieldName($params) {
    $res = array();
    $table = strtolower($params['table']);
    if(in_array($table,array('store','country','currency','measure'))){
        $res1 = read_file_one_value(BASE_DATA_PATH.'/cache/'.$table.'.php',$params['field'],$params['value'],$params['fields']);
        if($res1){
            return $res1;
        }else{
            $res2 = Model()->table($table)->select();
            write_file(BASE_DATA_PATH.'/cache/'.$table.'.php',$res2);
            $res = DB::getRow(array('table' => $params['table'], 'field' => $params['field'], 'value' => $params['value']), $params['fields']);
        }
    }else{
        $res = DB::getRow(array('table' => $params['table'], 'field' => $params['field'], 'value' => $params['value']), $params['fields']);
    }
    return $res ? $res[$params['fields']] : '--';
}

/**
 * 格式化时间
 * @param type $strtime 时间戳
 * @param type $s
 * @return string（如：2015-12-10 12:10:02）
 */
function format_time($strtime = '', $s = 's') {
    if (!$strtime || $strtime == 0) {
        return '--';
    }
    if (strtolower($s) == 'd') {
        return date('Y-m-d', $strtime);
    } elseif (strtolower($s) == 'i') {
...
/**
   ...
   function upload_file($field,$folder='') {
       ...
       if(!in_array($ext,array('jpg','gif','png'))){
           return array('status'=>0,'msg'=>'文件格式不正确！');
       }
       //最大10M
       if($upload_file['size'] > 1024 * 1024 * 10){
           return array('status'=>0,'msg'=>'文件太大！');
       }
       $path = BASE_ROOT_PATH.DS.DIR_UPLOAD.DS;
       if($folder){
           $path = BASE_ROOT_PATH.DS.DIR_UPLOAD.DS.$folder.DS;
       }
       mk_dir($path);
       $new_file = $path .date('YmdHis').mt_rand(10000, 99999).'.'.$ext;
       if(move_uploaded_file($upload_file['tmp_name'],$new_file)){
           return array('status'=>1,'path'=>str_replace(BASE_ROOT_PATH,'',$new_file));
       }  
   }
