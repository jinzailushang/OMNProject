    ...
    protected function parse_options($options = array()) {
        ...
        if ($options['where'] === true) {
            ...
            foreach ($options['where'] as $key => $val) {
                if ($this->table_name == '')
                    break;
                if (strpos($key, '|') || strpos($key, '&') || in_array($key, array('_op')))
                    break; //除去 name|title&nickname和自定义运算符的情况
...
//                $data[$this->get_pk()]  = $insertId;
     ...
     * @return string
     */
    public function showpage($style = null,$func=null) {
        return pagecmd('show', $style,$func);
    }
    ...
    public function saveUploadPic($base64_image_content) {
        if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
            $type = $result[2];
            //检测文件夹是否存在，如果不存在则创建
            $this->mkdirs(BASE_ROOT_PATH.DS.DIR_UPLOAD);
            $file_path = DS.DIR_UPLOAD.DS.date("YmdHis").rand(10000,99999).".".$type;
            $new_file = BASE_ROOT_PATH.$file_path;  //保存路径
    ...
    public function getImgPath($image) {
        if($image && file_exists(BASE_ROOT_PATH.$image)){
            return C('pcm_site_url').$image;
        }
        return '';
    }
...
class ModelDb {
    ...
    public function select($options = array()) {
        ...
        if ($options['cache'] !== false) {
            ...
            if (isset($_cache[$key])) {
                //return $_cache[$key]; //去除组缓存20150722
            }
        }
    ...
    public function buildSelectSql($options = array()) {
        ...
        //file_put_contents('sql.log', print_r($sql,true).PHP_EOL,FILE_APPEND);
        return $sql;
    }

    public function parseSql($sql, $options = array()) {
        $sql = str_replace(
                array('%TABLE%', '%DISTINCT%', '%FIELD%', '%JOIN%', '%WHERE%', '%GROUP%', '%HAVING%', '%ORDER%', '%LIMIT%', '%UNION%', '%INDEX%'), array(
    ...
    protected function parseWhereItem($key, $val) {
        ...
        if (is_array($val)) {
            if (is_string($val[0])) {
                if (preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT|NOTLIKE|LIKE)$/i', $val[0])) { // 比较运算
                ...
                } elseif (preg_match('/TIMEHIS/i', $val[0])) {
                    $data = is_string($val[1]) ? explode(',', $val[1]) : $val[1];
                    if ($data[0] && $data[1]) {
                        $whereStr .= ' (' . $key . ' BETWEEN ' . $this->parseValue($data[0]) . ' AND ' . $this->parseValue($data[1]) . ' )';
                    } elseif ($data[0]) {
                        $whereStr .= $key . ' ' . $this->comparison['gt'] . ' ' . $this->parseValue($data[0]);
                    } elseif ($data[1]) {
                        $whereStr .= $key . ' ' . $this->comparison['lt'] . ' ' . $this->parseValue($data[1]);
                    ...
                    $data = is_string($val[1]) ? explode(',', $val[1]) : $val[1];
                    if ($data[0] && $data[1]) {
                        $if_time = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $data[1]);
                        if($if_time){
                            $endt = "'".date("Y-m-d",strtotime($data[1])+86400)."'";
                            $whereStr .= ' (' . $key . ' BETWEEN ' . $this->parseValue($data[0]) . ' AND ' . $endt . ' )';
                        }else{
                        ...
                        if($if_time){
                            $endt = "'".date("Y-m-d",strtotime($data[1])+86400)."'";
                            $whereStr .= $key . ' ' . $this->comparison['lt'] . ' ' . $endt;
                        }else{
