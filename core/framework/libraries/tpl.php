...
class Tpl {
    ...
    /**
     * 使用smarty输出页面
     * @copyright (c) 2016-01-26, jack
     * @param $page_name:  模板文件名称
     * @param $ajax:  是否为ajax请求输出
     * @return : 输出的内容
     */
    public static function showpageSMARTY($page_name = '', $layout = '', $ajax = false) {
        global $tpl;
        if (!defined('SMARTY_TPL_NAME'))
            define('SMARTY_TPL_NAME', 'smarty/default');
        self::getInstance();
        if (!empty(self::$tpl_dir)) {
            $tpl_dir = self::$tpl_dir . DS;
        }
        //默认是带有布局文件
        if (empty($layout)) {
            $layout = 'inc/default_layout.tpl';
        } else {
            $layout = 'inc' . DS . $layout . '.tpl';
        }
        $layout_file = BASE_PATH . '/templates/' . SMARTY_TPL_NAME . DS . $layout;

        $tpl_file = BASE_PATH . '/templates/' . SMARTY_TPL_NAME . DS . $tpl_dir . $page_name . '.tpl';
        if (file_exists($tpl_file)) {
            //对模板变量进行赋值
            $output = self::$output_value;
            //页头
            ...
            if ($layout != '') {
                if (file_exists($layout_file)) {
                    $tpl->assign('tpl_file', $tpl->fetch($tpl_file));
                    if ($ajax) {
                        echo $tpl->fetch($layout_file, 1);
                    } else {
                        $tpl->display($layout_file, 1);
                    }
                } else {
                    $error = 'Tpl ERROR:' . 'templates' . DS . $layout . ' is not exists';
                    throw_exception($error);
                ...
                if ($ajax) {
                    return $tpl->fetch($tpl_file, 1);
                } else {
                    $tpl->display($tpl_file, 1);
                }
            }
        } else {
            $error = 'Tpl ERROR:' . $tpl_file . ' is not exists';
            throw_exception($error);
        }
    }
