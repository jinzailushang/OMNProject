...
class apiControl
{
  ...
  public function __construct()
  {
    ...
    if(empty($this->post_data['customer_name']) || empty($this->post_data['sign']) || empty($this->post_data['qTime']) ){
        $this->output(0,'参数缺失！请检查！');
    }
    if(time() - $this->post_data['qTime'] > 300){
        $this->output(0,'非法请求！');
    }

    $sign = md5($config['wms1']['name'].$config['wms1']['key'].$this->post_data['qTime']);

    if($this->post_data['sign'] !=  $sign){
        $this->output(0,'签名错误！');
    }
  }
