<?php

/**
 * Sms sdk
 *
 * @property string $serverUrl
 * @property string $appKey
 * @property string $secret
 */
class Sms {

  private $serverUrl,
    $appKey,
    $secret;

  public function __construct($serverUrl = NULL, $appKey = NULL, $secret = NULL)
  {
    if ($serverUrl) {
      $this->serverUrl = $serverUrl;
    }
    if ($appKey) {
      $this->appKey = $appKey;
    }
    if ($secret) {
      $this->secret = $secret;
    }
  }

  public function __set($name, $value)
  {
    if (property_exists($this, $name)) {
      $this->$name = $value;
    }
  }

  public function checkCode($phone, $code, $destroy = TRUE)
  {
    $params = array(
      'action' => __FUNCTION__,
      'phone' => $phone,
      'content' => $code,
      'destroy' => $destroy
    );
    return $this->post($params);
  }

  public function send($phone, $content, $type = 'content', $channel = NULL, $outerId = NULL, $specifiedTime = 0, $expire = 0)
  {
    $params = array(
      'action' => __FUNCTION__,
      'phone' => $phone,
      'content' => $content,
      'type' => $type,
      'specifiedTime' => $specifiedTime,
      'outerId' => $outerId,
      'channel' => $channel,
      'expire' => $expire
    );
    return $this->post($params);
  }

  public function sendContent($phone, $content, $channel = NULL, $outerId = NULL, $specifiedTime = 0)
  {
    $params = array(
      'action' => 'send',
      'phone' => $phone,
      'content' => $content,
      'type' => 'content',
      'specifiedTime' => $specifiedTime,
      'outerId' => $outerId,
      'channel' => $channel
    );
    return $this->post($params);
  }

  public function sendCode($phone, $channel = NULL, $expire = 3600)
  {
    $params = array(
      'action' => 'send',
      'phone' => $phone,
      'type' => 'code',
      'channel' => $channel,
      'expire' => $expire
    );
    return $this->post($params);
  }

  public function get($phone, $outerId = NULL)
  {
    $params = array(
      'action' => __FUNCTION__,
      'outerId' => $outerId,
      'phone' => $phone
    );
    return $this->post($params);
  }

  private function restObj()
  {
    if (!class_exists('restclient')) {
      require_once __DIR__ . '/restclient.php';
    }
    return new \restclient;
  }

  private function genRequestToken($time = 0)
  {
    return md5($this->appKey.'#' . ($time ? $time : time()) .'#'. $this->secret);
  }

  private function post($params)
  {
    $rest = $this->restObj();
    $rest->url = $this->serverUrl;
    $time = time();
    $rest->params = array_merge($params, array(
      'time' => $time,
      'app_key' => $this->appKey,
      'token' => $this->genRequestToken($time)
      )
    );

    $rest->post();
    return json_decode($rest->response);
  }

}
