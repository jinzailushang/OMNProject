...
<div class="row-file">
    <div class="file-form">
    	<div class="file-form-list">
            <form id="add_form" method="post" enctype="multipart/form-data" action="<?php echo urlShop('order','order_import')?>" target="formsme">
                <input type="hidden" name="form_submit" value="ok" />
                <input type="hidden" name="type" value="<?php echo $_GET['type']?>" />
            	<p class="file-p">请按照数据模板的格式准备导入数据，模板中的表头名称不可更改，表头行不能删除。</p>
                <div class="fileBtn"><a href="<?php echo urlShop('order', 'upload_eg',array('type'=>$_GET['type']));?>" target="_blank" class="downloadBtn"><i class="ico-but"></i>下载模板</a></div>
...
function callback(res) {
    $('#loading-mask').hide();
    if (res.status == 1) {
        var html = '<p style="color:green">成功导入'+res.snum+'条！</p><p style="color:red">失败'+res.fnum+'条！</p><p>'+(res.error!=undefined&&res.error!='undefined'&&res.error||'')+'</p>';
        $('.tips').html(html);
    } else {
