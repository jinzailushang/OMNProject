      ...
      if (res.status == 1) {
        ...
        for (var i in res.data) {
          ...
          if (status == 0) {
            ...
            // 判断是否要缴税
            if (status == 40 && (parseFloat(r.tariff_fee) == 0 || parseInt(r.pay_tariff_status) == 1)) {
...
function fill_tc(tid,ship_method) {
  ...
  $.getJSON(url, {tid: tid,ship_method:ship_method}, function (res) {
    if (res.status) {
      ...
      if(ship_method){
         $('#fee_type').text('服务收费：首重'+res.channel_info.first_weight+'kg，续重'+res.channel_info.first_weight+'kg，首重费￥'+res.channel_info.first_weight_fee+'，续重费￥'+res.channel_info.continue_weight_fee);
         }
      // extra_service_fee
